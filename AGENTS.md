...
- **Prompt review gate:** Do not execute still-gen, video-gen, I2V, extend, or image-edit until Salvatore has approved the exact prompt/edit text in chat. Before every video run, send workflow name + ID, API, model, still URL, duration, and the exact motion prompt, then wait for approve or deny. See `.cursor/skills/prompt-review-gate/SKILL.md`.
