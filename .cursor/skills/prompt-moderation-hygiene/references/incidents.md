...
| 2026-09-17 | `Vid_gen_lab_scenes` 2251 | xAI `grok-imagine-image-2.0` | PBVita-Lab-207 `compound_name` Wolverine | lab vial still, no people | `… compound name … reading exactly 'Wolverine' …` (~9533 chars `video_prompt`) | `Generated image rejected by content moderation.` | word | print name → `BPC-157/TB-500`; `choose_compound` / `compound_name` stay Wolverine; `alias_stack_names` no longer rewrites `BPC-157/TB-500` back to Wolverine in prompt fields | **pending** Salvatore smoke — do not re-run grok from this agent |
