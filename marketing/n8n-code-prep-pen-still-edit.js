...
//
// still_url is runtime from save_still_url / grok_imagine_pen_still.
// still_edit_prompt: this-run still_edit_instructions first, then Sheet 14 via pick_pen_creation.
// Empty sheet / Set cell throws — do not invent a fallback prompt.
...
// This-run Set node wins so you can type an edit after seeing the still.
...
if (!sourceStill) {
  throw new Error(
    'prep_still_edit: need https still_url from grok_imagine_pen_still / save_still_url'
  );
}
if (!editPrompt) {
  throw new Error(
    'still_edit_prompt missing. Put it on Sheet 14 or type it in still_edit_instructions (creation_id=' +
      (creationId || '?') +
      ')'
  );
}
