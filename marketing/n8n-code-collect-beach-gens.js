// n8n Code node: collect_beach_gens
// Workflow: gen_film_beach_stills
// Mode: Run Once for All Items
// After: grok_imagine_still
// Before: sheets_update_still
//
// Appends generated https URLs onto each still_id take_urls.
...
var preps = [];
try {
  preps = $('prep_beach_gens').all();
} catch (e) {
  preps = [];
}

if (!grokItems.length) {
  throw new Error('collect_beach_gens: no items from grok_imagine_still.');
}
if (preps.length !== grokItems.length) {
  throw new Error(
    'collect_beach_gens: prep/grok count mismatch (' +
      preps.length +
      '/' +
      grokItems.length +
      ').'
...
for (var i = 0; i < grokItems.length; i++) {
  ...
  if (!stillId) {
    throw new Error('collect_beach_gens: still_id missing on prep item ' + i);
  }
  if (!groups[stillId]) {
    groups[stillId] = {
  ...
  if (u) groups[stillId].generated.push(u);
}
...
for (var g = 0; g < ids.length; g++) {
  var id = ids[g];
  var generated = groups[id].generated;
  if (!generated.length) {
    throw new Error(
      'collect_beach_gens: no https URLs from grok_imagine_still for ' + id
    );
  }
  var joined = generated.join(' | ');
  var prior = groups[id].prior;
  out.push({
    json: {
      still_id: id,
      generated_take_urls: joined,
      take_urls: prior ? prior + ' | ' + joined : joined,
      gen_count_this_run: generated.length,
    },
  });
}
