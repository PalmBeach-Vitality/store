...
… → save_creatomate_url
  ...
  → Buffer_post_IG          (names may match your other WF)
  → Buffer_post_FB
  → sheets_update_buffer    ← mark used_in_buffer = yes (optional)
```

Stories (`Buffer_IG_story` / `Buffer_FB_story`) are optional — copy only if you still want them.

---

## What to copy

From the **other / live** workflow, select + copy the Buffer chain only, e.g.:

- `buffer_ig_reel`
- `buffer_fb_reel`
- `Buffer_post_IG`
- `Buffer_post_FB`
- (optional) `Buffer_IG_story`, `Buffer_FB_story`
