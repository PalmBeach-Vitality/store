...
// Workflow: overlay_film_i2v_stack (one-shot, unpublished)
...
var MODELS = { seedance: 'bytedance/seedance-2.5', kling: 'kwaivgi/kling-v3.0-pro', veo: 'google/veo-3.1', runway: 'gen4.5' };
var START_URLS = { seedance: 'https://openrouter.ai/api/v1/videos', kling: 'https://openrouter.ai/api/v1/videos', veo: 'https://openrouter.ai/api/v1/videos', runway: 'https://api.dev.runwayml.com/v1/image_to_video' };
var RESOLUTIONS = { seedance: '1080p', kling: '1080p', veo: '1080p', runway: '1080p' };
var WORKFLOWS = {
...
if (!rows.length) throw new Error('overlay_film_i2v_stack: no rows from get_film_stills.');
var out = [];
for (var i = 0; i < rows.length; i++) {
  var stillId = String((rows[i] || {}).still_id || '').trim();
  if (!stillId) continue;
  var spec = STACK[stillId];
  if (!spec) throw new Error('overlay_film_i2v_stack: missing stack row for ' + stillId);
  var provider = spec[0];
  out.push({ json: {
    still_id: stillId,
...
if (out.length !== 25) throw new Error('overlay_film_i2v_stack: expected 25 rows, got ' + out.length);
return out;
