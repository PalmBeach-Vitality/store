...
const howto = sticky({
  config: {
    ...
    parameters: {
      ...
      width: 860,
      height: 360,
      content: '# edit_film004_vial_reach (unpublished)\n# FILM-004 only. Grok-edits the old FILM-001 reach still onto the FILM-014 alien beach with one MOTS-C vial.\n# Does not touch FILM-001. Run after overlay_film004_vial_reach. Do not Publish.',
    },
  },
});
...
const getFilm = node({
  ...
  config: {
    ...
    output: [{ still_id: 'FILM-004', still_edit_prompt: 'Keep this exact reach pose', model_still: 'grok-imagine-image', aspect_ratio: '9:16', n: '1', picked_url: 'https://raw.githubusercontent.com/PalmBeach-Vitality/store/cursor/film004-vial-reach-4c4b/marketing/stills/film001-beach-reach.png' }],
  },
});
...
const grokEdit = node({
  ...
  config: {
    ...
    output: [{ data: [{ url: 'https://example.com/film004-edit.png' }] }],
  },
});
...
const sheetsUpdate = node({
  ...
  config: {
    ...
    output: [{ still_id: 'FILM-004', take_urls: 'https://example.com/film004-edit.png' }],
  },
});

export default workflow('edit_film004_vial_reach', 'edit_film004_vial_reach')
  .add(howto)
  .add(startTrigger)
  .to(getFilm)
  .to(prepEdits)
