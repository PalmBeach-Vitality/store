...
const getFilmStills = node({
  ...
  config: {
    ...
    position: [240, 304],
    output: [{ still_id: 'FILM-009', status: 'Active', picked_url: 'https://example.com/still.jpg', video_url: '', video_provider: 'kling' }],
  },
});
...
const falI2vGenerate = node({
  ...
  config: {
    ...
    parameters: {
      resource: 'model',
      operation: 'generate',
      model: { __rl: true, mode: 'id', value: expr('{{ $json.model_video }}'), cachedResultName: 'kling' },
      modelParameters: {
        parameters: [
          { parameter: 'prompt', value: expr('{{ $json.video_motion_prompt }}') },
          { parameter: 'start_image_url', value: expr('{{ $json.still_url }}') },
...
export default workflow('film_i2v_kling', 'film_i2v_kling')
  .add(howto)
  .add(manualTrigger)
  .to(getFilmStills)
  .to(pickFilmStill)
