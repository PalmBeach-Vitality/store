...
const getFilmStills = node({
  ...
  config: {
    ...
    position: [240, 304],
    output: [{ still_id: 'FILM-009', status: 'Active', picked_url: 'https://example.com/still.jpg', video_url: '', video_provider: 'seedance' }],
  },
});
...
const falI2vGenerate = node({
  ...
  config: {
    ...
    parameters: {
      resource: 'model',
      operation: 'generate',
      model: { __rl: true, mode: 'id', value: expr('{{ $json.model_video }}'), cachedResultName: 'seedance' },
      modelParameters: {
        parameters: [
...
export default workflow('film_i2v_seedance', 'film_i2v_seedance')
  .add(howto)
  .add(manualTrigger)
  .to(getFilmStills)
  .to(pickFilmStill)
