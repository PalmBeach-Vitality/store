...
// still_edit_prompt: this-run still_edit_instructions first, then the Sheet via pick_creation.
...
if (!editPrompt) {
  throw new Error(
    'still_edit_prompt missing. Put it on Sheet 9 or type it in still_edit_instructions (creation_id=' +
      (creationId || '?') +
      ')'
  );
}
