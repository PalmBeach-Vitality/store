...
6. Only after that: write real copy on `LN-001`, set `status` to `ready`, run the send workflow again. It emails every **subscribed** row one at a time.
...
Empty cells fail. Placeholder draft copy (`Write the Lab Notes body here…`) also fails. Do not leave `from_email` / `reply_to` as anything except `info@palmbeach-vitality.com`.
...
| `subject` | yes | Short, not shouty. Avoid ALL CAPS and “FREE!!!” |
| `preview_text` | yes | Inbox snippet |
...
| `heading` | yes | H1 inside the branded wrapper |
| `body_text` | yes | Plain text. One paragraph per line. The workflow wraps HTML — do not paste a full HTML page |
| `cta_label` | yes | One button |
| `cta_url` | yes | One link. Keep total links low (logo + CTA + unsubscribe) |
| `logo_url` | yes | Hosted on the store |
| `shop_url` | yes | |
| `test_email` | yes | Your inbox for `status=test` |
| `delay_seconds` | yes | **5 or more**. Default `12`. This is how we stay under Gmail blast patterns |
